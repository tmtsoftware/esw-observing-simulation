...
object ADCCommand {
  ...
  val PrismPositionKey: GChoiceKey = PrismPosition.makeChoiceKey("position")

  private def getPosition(setup: Setup, key: GChoiceKey): Either[CommandIssue, PrismPosition] = {
    val cmdName = setup.commandName.name
    for {
      param          <- setup.get(key).toRight(MissingKeyIssue(s"$PrismPositionKey not found in command: $cmdName"))
  ...
  def getPrismPosition(setup: Setup): Either[CommandIssue, PrismPosition] =
    getPosition(setup, PrismPositionKey)
}
