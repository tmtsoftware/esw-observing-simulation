...
class ImagerDetectorTest extends ScalaTestFrameworkTestKit(EventServer) with AnyFunSuiteLike {
  ...
  detectors.foreach{ detectorPrefix => {
    test(s" ${detectorPrefix} - test the whole cycle of an observation | ESW-552") {
    ...
    test(s"${detectorPrefix} - behaviour should return Invalid when concurrent (Start Exposure) commands received | ESW-552") {
