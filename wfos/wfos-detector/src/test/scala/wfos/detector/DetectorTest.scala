  ...
  detectors.foreach { detectorPrefix =>
    {
      test(s" $detectorPrefix - test the whole cycle of an observation | ESW-558, ESW-559") {
        ...
        val exposureId   = ExposureId(s"$obsId-WFOS-IMG-DRK1-0023")
        ...
        val dataSubscription = eventService.defaultSubscriber.subscribeActorRef(
          Set(EventKey(detectorPrefix, ObserveEventNames.OpticalDetectorExposureData)),
      ...
      test(
        s"$detectorPrefix - behaviour should return Invalid when concurrent (Start Exposure) commands received | ESW-558, ESW-559"
      ) {
