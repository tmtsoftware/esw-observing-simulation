...
import csw.params.core.models.Coords.{AltAzCoord, BASE, Coord, EqCoord, EqFrame}
import esw.sm.api.protocol.StartSequencerResponse.Started
import csw.params.core.models.{Angle, ExposureId, ProperMotion}
...
import esw.agent.akka.app.process
...
//      val script = Paths.get(getClass.getResource("/test-setup.sh").toURI)
      //curl or wget to target
//      processExecutor.runCommand(List(script.toString, tcsContainerConfPath.toString), Prefix(TCS, "TcsContainer")).rightValue

      //spawn the iris container
      containerCmd = Some(ContainerCmd.start("iris_container_cmd_app", IRIS, List("--local", irisContainerConfPath.toString).toArray))
...
//      val tcsPkAssemblyLocation: Option[AkkaLocation] =
//        locationService.resolve(TestData.tcsPkAssemblyConnection, 15.seconds).futureValue
//      tcsPkAssemblyLocation.isDefined shouldBe true



      // send slewToTarget to tcs.pkAssembly
...
//        eqCoord
//      )
//      val slewToTargetCommand = Setup(Prefix(TCS, "test_client"), CommandName("SlewToTarget")).add(posParam)
//      val tcsAssembly = CommandServiceFactory.make(tcsPkAssemblyLocation.get)
